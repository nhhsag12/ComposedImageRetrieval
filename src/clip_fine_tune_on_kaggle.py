...
from data_utils import base_path, squarepad_transform, targetpad_transform, CIRRDataset, FashionIQDataset, input_path, output_path, source_path
...
def clip_finetune_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    training_path: Path = Path(
        output_path / f"models/clip_finetuned_on_fiq_{clip_model_name}_{training_start}")
...
def clip_finetune_cirr(num_epochs: int, clip_model_name: str, learning_rate: float, batch_size: int,
    ...
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        output_path / f"models/clip_finetuned_on_cirr_{clip_model_name}_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)
