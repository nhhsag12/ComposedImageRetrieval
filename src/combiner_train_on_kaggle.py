...
from data_utils import base_path, squarepad_transform, FashionIQDataset, targetpad_transform, CIRRDataset, input_path, output_path, source_path
...
def combiner_training_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    training_path: Path = Path(
        output_path / f"models/combiner_trained_on_fiq_{clip_model_name}_{training_start}")
...
def combiner_training_cirr(projection_dim: int, hidden_dim: int, num_epochs: int, clip_model_name: str,
    ...
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        output_path / f"models/combiner_trained_on_cirr_{clip_model_name}_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)
...
if __name__ == '__main__':
    ...
    experiment.log_code(folder=str(source_path / 'src'))
